...
export async function getDefaultModelAndProvider(): Promise<{ model: string; provider: string }> {
    ...
    if (cfg.providerMode === "rowboat") {
        return { model: cfg.model || SIGNED_IN_DEFAULT_MODEL, provider: "rowboat" };
    }
    if (cfg.providerMode === "chatgpt-codex") {
        return { model: cfg.model, provider: "chatgpt-codex" };
    }
    if (await isSignedIn()) {
...
export async function resolveProviderConfig(name: string): Promise<z.infer<typeof LlmProvider>> {
    ...
    if (cfg.providerMode === "byok" && cfg.provider.flavor === name) {
        return cfg.provider;
    }
...
export async function getKgModel(): Promise<string> {
    ...
    if (cfg.providerMode === "rowboat") return SIGNED_IN_KG_MODEL;
    if (cfg.providerMode === "chatgpt-codex") return cfg.knowledgeGraphModel ?? cfg.model;
...
export async function getTrackBlockModel(): Promise<string> {
    const cfg = await container.resolve<IModelConfigRepo>("modelConfigRepo").getConfig();
    if (cfg.providerMode === "rowboat") return SIGNED_IN_TRACK_BLOCK_MODEL;
    if (cfg.providerMode === "chatgpt-codex") return cfg.trackBlockModel ?? cfg.knowledgeGraphModel ?? cfg.model;
...
export async function getMeetingNotesModel(): Promise<string> {
    const cfg = await container.resolve<IModelConfigRepo>("modelConfigRepo").getConfig();
    if (cfg.providerMode === "rowboat") return SIGNED_IN_DEFAULT_MODEL;
    if (cfg.providerMode === "chatgpt-codex") return cfg.meetingNotesModel ?? cfg.model;
