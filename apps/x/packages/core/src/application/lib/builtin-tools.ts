    ...
    'LLMParse': {
        ...
        execute: async ({ path: filePath, prompt }: { path: string; prompt?: string }) => {
            try {
                ...
                const modelId = activeProvider.mode === 'byok' ? modelConfig.model : activeProvider.defaultModel;
                const providerName = modelConfig.providerMode === 'byok' ? modelConfig.provider.flavor : activeProvider.mode;
